...
export default function DomeGallery({
  ...
  // Native tap detection - bypasses gesture library entirely
  useEffect(() => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (dt < 400 && dist < 8) {
        ...
        const tileImage = target.closest('.item__image') as HTMLElement;
        if (tileImage && !tileImage.classList.contains('item__image--reference')) {
          openItemFromElement(tileImage);
        }
      }
    };

    sphere.addEventListener('pointerdown', onDown, true);
    sphere.addEventListener('pointerup', onUp, true);
    return () => {
      sphere.removeEventListener('pointerdown', onDown, true);
      sphere.removeEventListener('pointerup', onUp, true);
    };
  }, [openItemFromElement]);
